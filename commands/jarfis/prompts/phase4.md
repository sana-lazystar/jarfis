...
─── TDD Green Phase + Ratchet (Step 4-0.5 실행 시) ───
Phase 4-0.5에서 QA가 작성한 테스트 코드가 존재합니다.
1. 각 태스크 구현 전, 해당 테스트를 실행하여 RED(실패) 상태를 확인하세요.
2. 태스크를 구현합니다.
3. 구현 후 **전체 테스트를 실행**하여 결과를 보고하세요:
   [TEST_RESULT: passed={N}, failed={N}, total={N}]
4. 테스트가 통과하지 않으면 구현을 수정하세요.

📊 **래칫 규칙**: 오케스트레이터가 테스트 통과율(passed/total)을 추적합니다.
   이전 태스크 완료 시점보다 통과율이 하락하면 재구현이 지시됩니다.
...
⚠️ 테스트 파일을 수정해야 하는 경우:
   커밋 메시지에 반드시 [TEST_MODIFIED: {파일명} — {사유}]를 포함하세요.
   Phase 5 QA에서 해당 수정의 타당성을 검증합니다.
