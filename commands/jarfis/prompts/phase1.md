...
> **래칫 규칙**: 한번 Pass(2점)된 항목은 PO 재작성 후에도 Pass를 유지해야 한다.

검증 체크리스트 (5개 항목, 각 0-2점):
1. **모호 표현** — "적절한/빠른/충분한/필요에 따라/추후 결정" 등 감지 → 0: 3개+, 1: 1-2개, 2: 모두 구체적 수치
2. **KPI 측정 가능성** — 각 KPI에 숫자(목표값) + 측정 방법 → 0: 없음/정성적, 1: 숫자만, 2: 숫자+측정방법
3. **Performance Budget 구체성** — 각 지표에 구체적 숫자 + 측정 방법 → 0: 미정의, 1: 일부만, 2: 전부
4. **Required Roles 근거** — 각 역할의 근거 칸이 1문장 이상 → 0: 없음, 1: 일부만, 2: 전부
5. **스코프 경계** — "Out of Scope" 섹션 → 0: 미정의, 1: 포함만, 2: 포함+제외

검증 결과 처리:
- **5개 항목 모두 Pass(2점)**: Gate 1로 직접 진행
- **Fail 항목 존재**: PO(senior-product-owner)에게 Fail 항목만 명시하여 PRD 수정 지시
  - **래칫 검증**: 재작성 후, 이전에 Pass였던 항목이 Fail로 변경되면 래칫 위반 → PO에게 "통과 항목 품질 유지하면서 Fail 항목만 개선하라" 재지시
  - 최대 2회 반복. 이후에도 Fail 항목 존재 시 Gate 1에서 "PRD Score: X/10, Fail: [항목]"으로 사용자에게 경고

**PO 재작성 지시 시 포함할 규칙** (채점 기준 자체는 전달하지 않음):
- "이전에 통과한 항목의 품질을 떨어뜨리지 마세요"
- "Fail로 지적된 항목만 집중적으로 개선하세요"
- "기존 PRD 구조를 유지하면서 해당 섹션만 보강하세요"
