...
## Phase 4 Agent Status (오케스트레이터 사전 작업)

> Phase 5 시작 전, 오케스트레이터는 `.jarfis-state.json`의 `phase4_agents`를 읽어
> 아래 정보를 각 리뷰 에이전트 프롬프트에 주입한다.
...
⚠️ "skipped" 파트의 코드는 존재하지 않으므로 리뷰 대상에서 제외하세요.
⚠️ "failed" 파트는 코드가 불완전할 수 있으므로 리뷰 시 이를 감안하세요.
───────────────────────────────
```

> phase4_agents 필드가 없으면 이 섹션을 생략하고, tasks.md의 N/A 기준으로 폴백한다.
...
⚠️ $DOCS_DIR/tasks.md를 확인하여 N/A가 아닌 (실제 구현된) 파트의 코드만 리뷰하세요. (+ Phase 4 Agent Status 참조)
...
⚠️ $DOCS_DIR/tasks.md를 확인하여 N/A가 아닌 (실제 구현된) 파트만 테스트하세요. (+ Phase 4 Agent Status 참조)
...
─── TDD 활성화 시 리뷰 경량화 (phases.4.tdd_enabled === true) ───
Phase 4-0.5에서 QA가 작성한 테스트가 이미 통과 상태입니다.
test-strategy.md의 P0/P1 기능 정합성 시나리오는 이미 검증되었으므로,
QA 리뷰는 아래에 집중하세요:
- 테스트가 커버하지 못한 통합/E2E 레벨 이슈
...
⚠️ $DOCS_DIR/tasks.md를 확인하여 N/A가 아닌 (실제 구현된) 파트만 리뷰하세요. (+ Phase 4 Agent Status 참조)
...
참조 문서 (수정 방향 판단 시):
- $DOCS_DIR/architecture.md — 원래 설계 의도 확인 (데이터 모델, API 패턴, ADR)
- $DOCS_DIR/tasks.md — 해당 태스크의 원래 요구사항 확인

⚠️ diagnosis.md의 수정 지시가 architecture.md의 설계 원칙과 충돌하는 경우,
   diagnosis.md의 지시를 따르되 충돌 사실을 커밋 메시지에 기록하세요.
