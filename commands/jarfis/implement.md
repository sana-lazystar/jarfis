...
이번 implement에서 수정/생성/삭제한 모든 JARFIS 파일을 repo에 반영한다:

- **수정/생성된 파일**: `~/.claude/` → `{repo_path}/`로 복사. 경로 매핑:
  - `~/.claude/commands/jarfis.md` → `{repo_path}/commands/jarfis.md`
  - `~/.claude/commands/jarfis/*` → `{repo_path}/commands/jarfis/*`
  - `~/.claude/agents/jarfis/*` → `{repo_path}/agents/jarfis/*`
  - `~/.claude/hooks/*` → `{repo_path}/hooks/*`
  - `~/.claude/scripts/*` → `{repo_path}/scripts/*`
  - `~/.claude/statusline-command.sh` → `{repo_path}/statusline-command.sh`
- **삭제된 파일**: repo에서도 삭제한다.
- **동기화 대상 외 제외**: `.jarfis-version`, `.jarfis-source`, `jarfis-learnings.md`, `.jarfis-state.json`, `.compact-backups/`, `.distill-backup/`는 복사하지 않는다 (로컬 전용 데이터).

동기화 후 결과를 표시한다:
```
🔄 Repo 동기화: {N}개 파일 → {repo_path}
```
