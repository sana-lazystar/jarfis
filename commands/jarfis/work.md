...
산출물은 `$JARFIS_WORKSPACE_DIR/works/{YYYYMMDD}/{작업물명}/` 디렉토리에 저장한다.
- `$JARFIS_WORKSPACE_DIR`: `~/.claude/.jarfis-works-dir` 파일에서 읽은 경로 (기본값: `~/.jarfis-workspace`)
...
- **프로젝트별 파일** (`project-profile.md`, `project-context.md`)은 각 프로젝트의 `.jarfis/`에 저장한다 (변경 없음).

예시: `~/.jarfis-workspace/works/20260224/게시판-에디터-고도화/prd.md`
...
0. **작업물명 입력 및 Git 브랜치 설정**
   ...
   **0-a. 작업물명 입력**
   ...
   - `$JARFIS_WORKSPACE_DIR`을 `~/.claude/.jarfis-works-dir` 파일에서 읽는다. 파일이 없으면 `~/.jarfis-workspace`을 기본값으로 사용한다.
   - `$DOCS_DIR`을 `$JARFIS_WORKSPACE_DIR/works/{YYYYMMDD}/$WORK_NAME`으로 결정한다 (절대경로).
   ...
   **0-a-4. Workspace Detection (프로젝트 구조 확인)**
   ...
   이 단계에서 프로젝트 구조를 확정하고 `.jarfis-state.json`의 `workspace` 필드를 즉시 기록한다.

   1. AskUserQuestion으로 프로젝트 구조를 확인한다:
      ```
      question: "이 작업의 프로젝트 구조를 선택해주세요."
      header: "Workspace"
      options:
        - label: "Monorepo"
          description: "BE/FE가 같은 프로젝트 (Next.js, Nuxt 등)"
        - label: "Multi-project"
          description: "BE/FE가 별도 프로젝트/레포"
        - label: "FE만 수정"
          description: "프론트엔드만 수정하는 작업"
        - label: "BE만 수정"
          description: "백엔드만 수정하는 작업"
      ```

   2. 선택별 처리:

      **[Monorepo]**:
      - CWD가 git repo인지 확인 (`git rev-parse --show-toplevel` 성공 여부).
      - git repo이면: `workspace.type = "monorepo"`, BE path = `"."`, FE path = `"."`
      - git repo가 아니면: AskUserQuestion으로 "프로젝트 경로를 입력하세요 (CWD 기준 상대경로 또는 절대경로):" → 입력받은 경로를 BE/FE 모두에 설정
      - 프레임워크 자동 감지: 해당 경로의 `package.json` dependencies에서 `next`→Next.js, `nuxt`→Nuxt, `vue`→Vue, `react`→React 등 감지

      **[Multi-project]**:
      - `workspace.type = "multi-project"`
      - AskUserQuestion으로 BE/FE 경로를 각각 입력받는다:
        ```
        "BE 프로젝트 경로를 입력하세요 (CWD 기준 상대경로 또는 절대경로):"
        "FE 프로젝트 경로를 입력하세요:"
        ```
      - 각 경로의 유효성 검증: 디렉토리 존재 여부 확인. 존재하지 않으면 재입력 요청.
      - 각 경로에서 프레임워크 자동 감지

      **[FE만 수정]**:
      - BE path = `"N/A"`, `workspace.type = "monorepo"`
      - CWD가 git repo이면: FE path = `"."`
      - git repo가 아니면: "FE 프로젝트 경로를 입력하세요:" → 입력값 설정
      - 프레임워크 자동 감지

      **[BE만 수정]**:
      - FE path = `"N/A"`, `workspace.type = "monorepo"`
      - CWD가 git repo이면: BE path = `"."`
      - git repo가 아니면: "BE 프로젝트 경로를 입력하세요:" → 입력값 설정
      - 프레임워크 자동 감지

   3. `.jarfis-state.json`의 `workspace` 필드를 기록한다:
      ```json
      {
        "workspace": {
          "type": "monorepo | multi-project",
          "projects": {
            "backend": { "path": ".", "framework": "nest.js" },
   ...
   4. `$BACKEND_PROJECT_DIR`, `$FRONTEND_PROJECT_DIR` 변수를 설정한다.
      - 상대경로인 경우 CWD 기준으로 해석한다.
      - `"N/A"`인 경우 빈 문자열로 설정한다.

   **0-b. Git 브랜치 동기화 및 생성**

   workspace 정보를 기반으로 Git 브랜치를 설정한다.

   **[monorepo]** (FE만/BE만 포함):
   - `$BACKEND_PROJECT_DIR` 또는 `$FRONTEND_PROJECT_DIR` 중 `"N/A"`가 아닌 경로에서 `git rev-parse --show-toplevel`로 git repo 확인.
   - git repo가 아니면 이 단계를 건너뛴다.
   ...
   **[multi-project]**:
   - BE/FE 각 경로에서 독립적으로 위 과정을 반복한다:
     - `cd $BACKEND_PROJECT_DIR && git rev-parse --show-toplevel` → git repo이면 브랜치 생성
     - `cd $FRONTEND_PROJECT_DIR && git rev-parse --show-toplevel` → git repo이면 동일 이름 브랜치 생성
   - `.jarfis-state.json`에 `branches` 필드로 기록한다:
     ```json
     { "branches": { "backend": "$WORK_NAME", "frontend": "$WORK_NAME" } }
     ```
   - 각 repo에서 uncommitted 변경사항/develop 브랜치 유무를 개별 확인한다.
...
**`workspace` 설정 규칙:**
- Phase 0의 Step 0-a-4에서 사용자 입력을 기반으로 `.jarfis-state.json`의 `workspace` 필드를 설정한다.
...
**`$JARFIS_WORKSPACE_DIR` 결정 규칙:**
- `~/.claude/.jarfis-works-dir` 파일이 존재하면 그 내용을 `$JARFIS_WORKSPACE_DIR`로 사용한다.
- 파일이 없으면 `~/.jarfis-workspace`를 기본값으로 사용하고 디렉토리를 자동 생성한다.
