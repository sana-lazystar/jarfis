...
> Do not edit manually. Last updated: 2026-05-06 (M5) | Version: 4.0.7 (M7 → 4.1.0)
...
└── jarfis/
    ...
    ├── skills/                    # Flat Skills catalog (v4 — relocated from domains/{web,desktop}/skills/; 17 skills, +react-native M5)
...
- `~/.claude/scripts/jarfis/` — Python module directory (referenced by `jarfis_cli.py`)
  ...
  - `compose/` — Compose CLI package (`__main__.py` + `assembler.py` + `config.py` + `models.py` + `reader.py` + `resolver.py` + `skills.py` + `validate.py`) — deterministic agent composition with 4-stage skill fallback, N-3 section-missing audit
