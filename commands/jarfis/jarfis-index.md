...
> Do not edit manually. Last updated: 2026-04-13 | Version: 3.5.0
...
└── jarfis/
    ...
    │   ├── phase1.md              # Phase 1 Discovery prompt + PO wiki reference + additional tasks + $MEETING_EXTRA injection + PRD Ratchet rules (209 lines)
    │   ├── phase2.md              # Phase 2&3 Architecture/UX prompt + wiki reference + HTML mockup (214 lines)
    ...
    │   ├── phase4.md              # Phase 4 Implementation prompt + TDD Step 4-0.5 + Ratchet + TEST_RESULT/TEST_MODIFIED reporting (254 lines)
    │   ├── phase4-5.md            # Phase 4.5 Operational Readiness + dev server check (37 lines)
    │   ├── phase5.md              # Phase 5 Review & QA + Phase 4 Agent Status injection + TDD lightweight + test_modifications validation + Fix original design reference + UX Designer dual comparison (305 lines)
...
| `/jarfis:project-init` | `jarfis/project-init.md` | Project analysis -> generate `./.jarfis/project-profile.md` |
...
- `{project_path}/.jarfis/project-profile.md` — Generated by project-init, referenced by work (inside each project)
- `{project_path}/.jarfis/project-context.md` — Context referenced during work execution (inside each project, optional)
- `$JARFIS_ORG_DIR/learnings.md` — Per-Org learning items (managed by upgrade, referenced by work. `.personal/orgs/{org}/learnings.md`)
...
- `~/.claude/scripts/jarfis_cli.py` — Python CLI entry point (single interface for all commands below)
  ...
  - `jarfis_cli.py preflight` — Pre-validation + Org auto-registration + project table auto-addition (on Org detection: unregistered in orgs.json -> auto-register, org-profile.md project table unregistered -> auto-add row)
