...
1. **래칫 이력 수집** (래칫 문서가 있는 경우):
   - `$ARGUMENTS`에 참조된 plan.md, brainstorm.md 등에서 "래칫 검증 이력" 섹션을 추출
   - ACCEPT/REJECT/DEFER 판정이 내려진 항목 목록을 `$RATCHET_HISTORY`로 정리
   - 래칫 문서가 없으면 `$RATCHET_HISTORY` = 빈 문자열 (래칫 필터링 스킵)
...
3. **Advocate 호출** (Agent tool, subagent_type: `jarfis-advocate`):
   - prompt: "다음 JARFIS 변경을 검토하세요: [변경안 요약]. 현재 시스템 상태: [index 요약]. 장점과 추가 제안을 분석하세요."

4. **Critic 호출** (Agent tool, subagent_type: `jarfis-critic`):
   - prompt에 래칫 이력을 **구체적으로** 포함:
     ```
     다음 JARFIS 변경을 검토하세요: [변경안 요약].
     Advocate 의견: [advocate 결과].

     ## 래칫 수렴 이력 (재소송 금지)
     아래 항목은 이미 래칫 검증으로 ACCEPT/REJECT/DEFER 판정이 내려졌다.
     이 결정을 다시 문제 삼는 것은 래칫 원칙 위반이다.
     신규 구현 디테일만 검토하라.

     $RATCHET_HISTORY

     약점과 리스크를 분석하되, 위 래칫 항목을 재소송하지 마세요.
     각 지적에 대해 "래칫 해당 여부"를 명시하세요.
     ```
   - `$RATCHET_HISTORY`가 비어있으면 래칫 관련 지시를 생략

5. **래칫 위반 필터링** (오케스트레이터가 Critic 결과 수신 후 직접 수행):
   - `$RATCHET_HISTORY`가 있을 때만 실행
   - Critic의 각 지적을 아래 체크리스트로 판정:
     | 체크 | 판정 |
     |------|------|
     | 지적이 래칫 이력의 ACCEPT 항목과 동일한 설계 결정을 문제 삼는가? | → 래칫 위반 — 무시 |
     | 지적이 plan.md/brainstorm.md에 이미 명시된 답이 있는 질문인가? | → 래칫 위반 — 무시 |
     | 지적이 구현 디테일(함수 시그니처, 에러 처리, 엣지 케이스)에 대한 것인가? | → 정당한 지적 — 채택 대상 |
   - 래칫 위반으로 판정된 지적은 합의안에 **반영하지 않는다**

6. **합의 판단**:
   - 정당한 지적만으로 합의 도출 (래칫 위반 지적은 제외)
...
🔒 래칫 필터링: [N개 지적 중 M개 래칫 위반 → 무시]
   [래칫 위반 항목 요약 (있으면)]
🤝 합의: [정당한 지적만 반영한 합의안 / 또는 "사용자 판단 필요"]
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
```
