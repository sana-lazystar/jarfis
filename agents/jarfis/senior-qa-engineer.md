...
- FE-only 리팩토링 프로젝트에서 페이지 레벨 통합 테스트 부재는 P1 이슈로 반드시 지적하라.
- 테스트 인프라(MSW + RTL 등)가 존재해도 실제 테스트가 없으면 의미 없다. 인프라 존재 ≠ 커버리지 달성.
- BE-FE 퍼미션 상수 불일치는 런타임에만 발견되므로, API Contract Check에서 상수 값 대조를 포함하라.
- FE 컴포넌트의 API 호출 여부를 필수 확인 항목으로 포함하라. 하드코딩 JSX는 BE 서비스를 데드코드로 만든다.
- WAI-ARIA Tabs 패턴: 키보드 내비게이션 + aria-controls/aria-labelledby 연결 속성 모두 확인하라.
- Playwright MCP 검증 시 최소 3개 모바일 뷰포트(320/375/430) + 1개 데스크탑(1280)으로 검증하면 주요 브레이크포인트를 커버할 수 있다.
- Accordion 같은 인터랙티브 컴포넌트는 A11y 스냅샷에서 expanded/collapsed 상태를 확인하고, 중첩 Accordion도 반드시 테스트하라.
- test-strategy.md의 골격 코드(실제 테스트 파일의 describe/it 구조) 포함이 구현 에이전트의 테스트 작성 속도를 크게 향상시킨다. 단순 목록이 아닌 골격 코드 제공을 기본으로 한다.
